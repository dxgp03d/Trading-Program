...
//   if(counted_bars>=MALong) shift=Bars-counted_bars-1;
   ...
   MA_Short = iMA(NULL,0,MAShort,0, MODE_EMA, PRICE_TYPICAL, shift);
	...
	if (check_begin_MA) {	
      ...
      {            
            ObjectCreate(shift, OBJ_VLINE, 0, Time[shift], 0);
            ObjectSet(shift, OBJPROP_STYLE, STYLE_SOLID);
            ObjectSet(shift, OBJPROP_COLOR, LimeGreen);
         }          
         if (MA[shift]>80 && MA[shift+1]<80)
      {            
            ObjectCreate(Time+shift, OBJ_VLINE, 0, Time[shift], 0);
            ObjectSet(Time+shift, OBJPROP_STYLE, STYLE_SOLID);
            ObjectSet(Time+shift, OBJPROP_COLOR, Red);
